...
# ── t_scsm_transition: Unified Verification & State Advancement ──
        current_idx = 0
        prev_node = UNDEFINED_NODE

        for path_idx, node in enumerate(path):
            # 1. Determine the exact physical arrival port for this specific logical hop
            if path_idx == 0:
                ing_port = self._get_link_port(self.id_to_name[node], "h1")
            else:
                # If returning from a VNF, the previous node is the current node
                if prev_node == node:
                    if node == 2: ing_port = self._get_link_port("s2", "h3")
                    elif node == 3: ing_port = self._get_link_port("s3", "h4")
                else:
                    ing_port = self._get_link_port(self.id_to_name[node], self.id_to_name[prev_node])

            # 2. Determine if this specific hop advances the service chain
            new_idx = current_idx
            if current_idx < len(chain) and chain[current_idx] == node:
                new_idx = current_idx + 1

            # 3. Fetch specific cryptographic key material for this hop
            key_mat = hop_keys.get(str(node), 0xDEADBEEF)

            # 4. Write the strict unified transition rule
            # NOTE: Assuming you kept the header name 'hdr.scsm' instead of 'hdr.vslice'
            self._write(
                node,
                table="SCSMIngress.t_scsm_transition",
                match_fields={
                    "hdr.scsm.slice_id": SLICE_ID,
                    "hdr.scsm.chain_id": cid,
                    "hdr.scsm.chain_idx": current_idx, # Paper calls it prog_idx
                    "hdr.scsm.prev_node": prev_node,
                    "std_meta.ingress_port": ing_port
                },
                action="SCSMIngress.action_execute_transition",
                params={
                    "new_chain_idx": new_idx,
                    "key_material": key_mat
                }
            )

            # Advance the simulation state for the next loop iteration
            current_idx = new_idx
            prev_node = node
        ...
        for node in path:
            ...
            if cname == "C1" and node == 2:
                if 2 not in fw_provisioned:
                    ...
                    p_fw  = self._get_link_port("s2", "h3")
            ...
            if cname == "C1" and node == 3:
                if 3 not in fw_provisioned:
                    # FIXED: S3 connects to S6, not S4!
                    p_s2  = self._get_link_port("s3", "s2")
                    p_ids = self._get_link_port("s3", "h4")
                    p_s6  = self._get_link_port("s3", "s6") 

                    # Arrives from S2 -> Send to NFIDS1 (h4)
                    self._write(3, table="SCSMIngress.forwarding",
                                match_fields={
                                    "hdr.ethernet.dst_addr": h2_mac, 
                                    "std_meta.ingress_port": (p_s2, 0x1FF)
                                },
                                action="SCSMIngress.forward", params={"port": p_ids}, priority=10)
                    
                    # Arrives from NFIDS1 -> Send to S6
                    self._write(3, table="SCSMIngress.forwarding",
                                match_fields={
                                    "hdr.ethernet.dst_addr": h2_mac, 
                                    "std_meta.ingress_port": (p_ids, 0x1FF)
                                },
                                action="SCSMIngress.forward", params={"port": p_s6}, priority=10)
                    fw_provisioned.add(3)
                continue  # Skip standard forwarding for S3
    ...
    def run_test_scenario(self, scenario_key: str):
        ...
        for path_idx, node in enumerate(path):
            ...
            # 1. Simulate the Unified Table Key Match (Prev Node check)
            if prev not in preds:
                print(f"    node {node:2d}: ✗ VIOLATION — Transition from {prev} denied!")
                ok = False
            else:
                print(f"    node {node:2d}: ✓ Transition OK (idx {current_idx})")

            # 2. Simulate State Advancement
            new_idx = current_idx
            if current_idx < len(chain) and chain[current_idx] == node:
                new_idx = current_idx + 1

            # 3. Simulate Tau Hash Update
            key = keys.get(str(node), 0xDEADBEEF)
            tau = compute_tau(tau, node, new_idx, key)
        ...
        if not ok:
            ...
            print(f"\n  → CHAIN BYPASS: chain_idx={current_idx} < k={k}")
        else:
